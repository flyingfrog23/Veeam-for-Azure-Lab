...
# using marketplace/vbazure.parameters.json, with env var overrides.
...
# Marketplace values can come from env vars (preferred) or fallback to vbazure.parameters.json
...
# Fallback values from parameters.json (only used if env vars not set)
...
# Use env vars if provided, otherwise fallback to file
...
if [[ -z "${PUBLISHER}" || -z "${OFFER}" || -z "${PLAN}" || -z "${PLAN_VERSION}" || -z "${APP_NAME}" || -z "${MRG_NAME}" ]]; then
  ...
  echo "       Provide via env vars (VBMA_*) or in ${PARAM_FILE}."
  exit 1
fi

# IMPORTANT: For Managed Applications, the managed resource group must NOT already exist.
# Azure will create it and attach management metadata. If it exists, you often get:
# InvalidApplicationManagedResourceGroupId
if az group exists -n "${MRG_NAME}" | grep -qi true; then
...
# Best-effort accept (different CLI versions / extensions vary)
...
# Extract appParameters.value as JSON.
# If it's empty object, omit --parameters entirely (avoids shell/CLI JSON parsing weirdness).
APP_PARAMS_JSON="$(jq -c '.parameters.appParameters.value // {}' "${SANITIZED_PARAM_FILE}")"

# Validate APP_PARAMS_JSON is valid JSON (defensive)
echo "${APP_PARAMS_JSON}" | jq -e . >/dev/null 2>&1 || {
  echo "ERROR: appParameters.value is not valid JSON in ${PARAM_FILE}"
  echo "       Value: ${APP_PARAMS_JSON}"
  exit 1
}
...
echo "    Managed resource group (created by Azure): ${MRG_NAME}"
echo "    Managed RG id: ${MRG_ID}"
